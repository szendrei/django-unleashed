...
class NewsLinkGetObjectMixin():
    ...
    def get_object(self, queryset=None):
        ...
        return get_object_or_404(NewsLink, slug__iexact=newslink_slug,
...
class PageLinksMixin:
    ...
    def _page_urls(self, page_number):
        return "?{pkw}={n}".format(pkw=self.page_kwarg,
                                   n=page_number)

    def first_page(self, page):
        if page.number > 1:    # don't show on first page
    ...
    def get_context_data(self, **kwargs):
        ...
        if page is not None:
            context.update({'first_page_url':self.first_page(page),
